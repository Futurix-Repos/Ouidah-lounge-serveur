    ...
    <section className="relative z-50 mt-8 pb-16" aria-labelledby="gallery-heading">
      ...
      >
        {products?.map((product: any) => (
          ...
          >
            <h3 className="text-center mt-6  font-semibold leading-8 tracking-tight text-white">
              {product.name}
            </h3>
    ...
    <div className="relative flex h-full flex-col space-y-4 ">
      <ul role="list" className="space-y-3 divide-gray-200 h-[60%] p-2 overflow-y-auto border">
        {items.map((item) => (
          <li
            key={item.id}
            className="flex flex-col items-start space-y-2 justify-start py-4 px-2 border shadow-xl bg-slate-100 rounded-sm"
          >
            <div className="flex">
              <div className="">
                <p className="text-xs truncate font-medium text-gray-900">{item.name}</p>
      ...
      <div className="mx-1 flex space-y-2 flex-col justify-center items-center w-full space-x-2">
        {mutation.isSuccess && <p>Commande réussie.</p>}
        {mutation.isError && (
          <div className="p-2 mt-6 flex items-center justify-between space-x-4 border w-full">
            <button
              onClick={async () => {
                mutation.mutate({orderId, items, amount: subTotal})
              }}
  ...
  return (
    <header className="w-full ">
      <div className="h-12 border mb-2 flex items-center justify-center text-lg font-bold">
        Commande pour la {table?.name}-{zone?.name}
      </div>
      <div className="relative border z-10 flex h-12 flex-shrink-0 border-b border-gray-200 bg-white shadow-sm">
...
export default function MenuBusyTableComponent() {
  return (
    <>
